...
import * as Animatable from 'react-native-animatable';   // <<== CHÈN VÀO ĐÂY
...
class Favorites extends Component {
  ...
  render() {
    if (this.props.dishes.isLoading) {
      ...
      const dishes = this.props.dishes.dishes.filter(
        (dish) => this.props.favorites.some((el) => el === dish.id)
      );

      return (
        <Animatable.View animation='fadeInRightBig' duration={2000}>   {/* <<== BỌC VÀO */}
          <SwipeListView
            data={dishes}
